...
FILTER_MIN_PROB = 0.55  # if enough names pass, treat LogReg as a filter then use RSI
...
def add_forward_label(df: pd.DataFrame, horizon: int = 20) -> pd.DataFrame:
    ...
    Label = 1 if forward return > POSITIVE_RETURN_THRESHOLD, else 0.
    """
    ...
    if "date" not in df.columns:
        ...
        if "date" not in df.columns:
            raise KeyError(
                ...
                "Ensure earlier preprocessing steps preserve 'date' (avoid dropping it via groupby.apply)."
            )
...
def prepare_dataset(data_dir: str) -> pd.DataFrame:
    """
    Full preprocessing pipeline for ML experiment.
    """
    ...
    # Fill missing fundamentals/RSI using only past information (no lookahead):
    # - forward-fill within ticker
    # - then fill remaining NAs cross-sectionally by date median (stable for ranks)
...
def train_logistic_model(train_df: pd.DataFrame) -> Tuple[Pipeline, List[str]]:
    """
...
def compare_portfolios(
    ...
    for dt, group in test_candidates.groupby("date"):
        ...
        # Use the logistic model primarily as a filter; if too few names pass,
        # fall back to taking the top probabilities directly.
        ...
        # apply a simple transaction cost per period (equal-weight portfolio)
...
def plot_roc_curve(model: Pipeline, test_df: pd.DataFrame, feature_cols: List[str], save_path="roc_curve.png") -> None:
    X_test = test_df[feature_cols]
    y_test = test_df["label"]
    probs = model.predict_proba(X_test)[:, 1]
...
def plot_cumulative_returns(
    ...
    ax.plot(filtered_curve.index, filtered_curve.values, label="VC2 + Logistic Filter")
    ax.set_ylabel("Cumulative Growth")
    ax.set_title("Baseline vs Logistic-Filtered VC2 (Rebalance Periods)")
    ax.legend()
...
def get_model_coefficients(model: Pipeline, feature_cols: List[str]) -> pd.Series:
    """
    Extract logistic regression coefficients (after scaling) for interpretability.
    """
    logreg = model.named_steps.get("logreg")
    if logreg is None or not hasattr(logreg, "coef_"):
        return pd.Series(dtype=float)
    coefs = pd.Series(logreg.coef_.ravel(), index=feature_cols).sort_values()
    return coefs
...
def run_experiment(data_dir: str = DATA_DIR) -> ExperimentResults:
    ...
    model, feature_cols = train_logistic_model(train_df)
    ...
    plot_roc_curve(model, test_df, feature_cols, save_path="roc_curve.png")
    plot_precision_recall(model, test_df, feature_cols, save_path="pr_curve.png")
...
if __name__ == "__main__":
    ...
    if not results.coefficients.empty:
        ...
        pos = results.coefficients[results.coefficients > 0].sort_values(ascending=False).head(5)
        neg = results.coefficients[results.coefficients < 0].sort_values().head(5)
        for name, val in pos.items():
            print(f"+ {name}: {val:.4f}")
        for name, val in neg.items():
            print(f"- {name}: {val:.4f}")
